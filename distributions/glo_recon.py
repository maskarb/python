...
def make_new_point(values, shift_factor, probs):
    ...
    paras = lm.distr.glo.lmom_fit(values)
    fitted_dist = lm.distr.glo(**paras)

    new_paras = copy.deepcopy(paras)
    new_paras['loc'] *= shift_factor
    new_dist = lm.distr.glo(**new_paras)

    new_probs = map_quantiles(new_dist, fitted_dist, probs)
